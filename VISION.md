...
| 11 | Slice reverso dinâmico | `flight why`: por que esse valor é isso? Cadeia mínima de escritas/aliasings recuada à origem, exata por aliasing+containment do grafo | ✅ **concluída** (`_slice.py`; aliasing/containment exatos, write exato em escopo, read-of name-level podado pelo ring) |
| 12 | `flight bisect` | Qual commit introduziu o bug — passivo (corpus por fingerprint) ou ativo (replay contra o código de cada commit via `git worktree`) | ✅ **concluída** (`_bisect.py`; commit carimbado na fita NONDET, worktree descartável, busca binária com skip) |
| 13 | Generalização do reprodutor | `flight generalize`: a fronteira em que cada valor load-bearing liga/desliga a falha (varredura 1-D sobre a fita) | ✅ **concluída** (`_generalize.py`; probe exponencial + bisseção, propriedade candidata + scaffold Hypothesis) |
| 14 | What-if no browser | `flight view --serve`: console what-if no navegador rodando o `what_if` real sobre a fita (caminho C cliente-fino) | ✅ **concluída** (`run_whatif` + servidor HTTP `/whatif`; fn resolvida da fonte embutida) |
| 15 | Diff visual + deep-links | `flight diff --html` (página estática lado a lado) + viewer WASM com modo compare (eixo de eventos) e deep-links por fragmento | ✅ **concluída** (`diff_html` testado; compare + `#step` no viewer, JS válido, index.html reconstruído) |
| 16 | Fix agêntico que **prova** o patch | `flight fix`: propõe um diff e reexecuta a fita com o patch — crash sumiu E sem divergência → VERIFIED (capstone) | ✅ **concluída** (`_agent.py`; ferramentas read-only incl. slice da Fase 11, provider heurístico offline + `--llm`, verificação sobre a fita) |
| 17 | Fleet mode | `flight serve`: coletor + índice SQLite + painel agregando milhares de `.flight`; detecção de regressão + grafo cross-service; linter de privacidade | ✅ **concluída** (`_fleet.py`; `/ingest`, top fingerprints, `↑ novo desde deploy`, `trace_graph`, `report_to`, `safe_to_send`) |
